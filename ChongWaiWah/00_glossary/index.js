...
function handleOnChange(event) {
  ...
  glossary
    .filter((item) => item.definition.toLocaleLowerCase().includes(searchvalue.toLowerCase()))
    .forEach((item) => {
      let p = document.createElement('p')
      p.innerHTML = getDisplayItem(item)
      div.append(p)
...
//Add or insert list of 'term' to dropbox
function binddropdown(){
  ...
  glossary     
    .forEach(
      ...
      //dropdown default value is first item
      index === 0 ? option.selected="Selected" : option.selected=""
 ...
 function dropdownSelected(event){
  div.innerHTML = ''
  glossary
    .filter((item) => item.term === event.target.value )
    .forEach((item) => {
      let p = document.createElement('p')
      p.innerHTML = getDisplayItem(item)
      div.append(p)
...
let checkboxLi =document.querySelector('.checkbox li')
function bindcheckbox(){
  checkbox.innerHTML='';
  checkboxLi.innerHTML=''
...
glossary.forEach((e,index)=>{
  e.tags.forEach((f,j) => {
    if ( !tagtmp.includes(f)){
      tagtmp.push(f)
...
tagtmp.forEach(
      ...
      myli.id = "li"+index
      mylabel.textContent = item
      mycheckbox.type='checkbox'
     // mycheckbox.name = item
      mycheckbox.text = item
      mycheckbox.id ="chk"+index
     // mycheckbox.name ="chk"+index
      ...
      mycheckbox.addEventListener('change',(e)=>{
          ...
          filtercheckbox(
          checkboxfilter(e,tagtmp)
          )
      })
      myli.append(mycheckbox)
      myli.append(mylabel)
  
      
      checkboxLi.append(myli );
 ...
 function checkboxfilter(event,val){
    let result = []
    val.forEach((element,index) => {
        var e = document.querySelector(".checkbox #chk"+index)
        console.log(event.target.id +" checked = "+e.checked)
        //checkbox checked
        if (e.checked){
          result.push(e.text)
        }
 ...
 function filtercheckbox(event){
  div.innerHTML = ''
  let result =[];
  let taglist =String( event).split(",");
  console.log("event value: "+ String( event).split(","))
  glossary    
    .forEach((item) => {
         if (item.tags.some(s=> taglist.includes(s))){
            result.push(item)
         }
    })
 ...
 function getDisplayItem(e){
    return `<b>${e.term}</b><br>
            ${e.definition}<br>
            
          <i style='color:blue'><b>tags : </b> ${e.tags}</i>
